...
def cor_selector(X, y):
    ...
    # # print(cor_feature)
    # # print(np.array(cor_list)[np.argsort(np.abs(cor_list))[-max_features:]].tolist())
    # with open('./feature_importance_pierxun.txt','w') as f:
...
def draw_pierxun(X):
    ...
    feat = ['MDEC-23', 'LipoaffinityIndex', 'BCUTc-1l', 'ATSc5', 'XLogP',
       'ATSc4', 'minHsOH', 'minsssN', 'ATSc3', 'VC-5', 'SPC-6', 'MLFER_A',
       'maxHsOH', 'ALogP', 'SdssC', 'hmin', 'minssCH2', 'MDEC-33',
       'BCUTc-1h', 'minHBint10']
    data = X[feat]
...
def caculate_score():
    ...
    # for idx,model in enumerate(model_name):
    #     path = 'feature_importance_' + model +'.txt'
    #     str_list = open(path,'r').readlines()
    #     score_list = [np.abs(float(item)) for item in str_list]
    #     mean = np.average(score_list)
    #     std = np.std(score_list)
    #     score_list = (score_list - mean) / std

    #     # with open('./normal_'+model+'.txt','w') as f:
    #     #     for item in score_list:
    #     #         f.write(str(item))
    #     #         f.write('\n')
            
    #     if len(result_score) == 0:
    #         result_score = weights[idx] *score_list
    #     else:
    #         result_score = np.sum([result_score, weights[idx] * score_list],axis=0)
    # pickle.dump(result_score,open('./result_socre'+'_'+str(weights[0])+'_'+str(weights[1])+'_'+str(weights[2])+'_'+str(weights[3])+'.pkl','wb'))
...
def select_byBiRelation():
    ...
    # new_score = divide_score.values.tolist()[0]
    # for idx,feat in enumerate(feat_name):
    #     if np.abs(new_score[idx]) > t:
    #         result.append(new_score[idx])
    #         columns_.append(feat)
    # new_data = pd.DataFrame([result],columns=columns_)
    
    # t = 0.05  ## 421
    # new_data.to_csv('./feature_entropy_pierxun_spearman_divide_score_select_'+str(t)+'.csv',index=False)
...
if __name__ == '__main__':
    ...
    # result = {}
    # result_feature = lgb_feature + pierxun_feature + spearman_feature + rf_feature
    ...
    #         result[item] += 1
    # print(result)
    # sort_result = sorted(result.items(),key=lambda item:item[1])
    # print(sort_result)
    # print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
